...
public class CategoriesController : ControllerBase { 
    ...
    public async Task<ActionResult> GetWithNotes(int id) {
        ...
        if (category is null)
            return NotFound(ApiError.NotFound($"Категория с id = {id} не найдена"));
        // Формируем ответ: категория + её неархивные заметки
        var response = new {
            Id = category.Id,
            Name = category.Name,
    ...
    [HttpPost]
    public async Task<ActionResult<ApiResponse<Category>>> Create([FromBody] 
CteateCategoryDto dto) {
        // [ApiController] автоматом вернёт 400 если модель невалидна
        // Но мы можем добавить доп проверки вручную
    ...
    public async Task<ActionResult> Delete(int id) {
        ...
        if (category is null)
            return NotFound(ApiError.NotFound($"Категория с id = {id} не найдена"));
        // Проверяем есть ли заметки в категории
